...
$lang->load('plg_djclassifiedspayment_djcfWebcash',JPATH_ADMINISTRATOR);
class plgdjclassifiedspaymentdjcfWebcash extends JPlugin
{
	function plgdjclassifiedspaymentdjcfWebcash( &$subject, $config)
	{
		parent::__construct($subject, $config);
		$this->loadLanguage('plg_djcfWebcash');
		$params["plugin_name"] = "djcfWebcash";
		$params["icon"] = "webcash_icon.png";
		$params["logo"] = "webcash_overview.png";
	...
	function _notify_url()
	{
		...
		$paypal_ipn = new webcash_ipn($paypal_info);
			...
			if(($status=='Completed') || ($status=='Pending' && $account_type==1)){				
				...
				if($payment){					
					$query = "UPDATE #__djcf_payments SET status='Completed',transaction_id='".$txn_id."' "
							."WHERE id=".$id." AND method='djcfWebcash'";					
					$db->setQuery($query);
					$db->query();
					...
					if($payment->type==2){										
						...
						$query = "INSERT INTO #__djcf_users_points (`user_id`,`points`,`description`) "
								."VALUES ('".$payment->user_id."','".$points."','".JText::_('COM_DJCLASSIFIEDS_POINTS_PACKAGE')." Webcash <br />".JText::_('COM_DJCLASSIFIEDS_PAYMENT_ID').' '.$payment->id."')";					
						$db->setQuery($query);
						$db->query();																		
					}else{
	...
	function process($id)
	{
		...
		{
			$urlpaypal="http://staging.webcash.com.my/wcgatewayinit.php";
		}
		elseif ($this->params["testmode"]=="0")
		{
			$urlpaypal="https://webcash.com.my/wcgatewayinit.php";
		}
		...
		// $form .='<input type="hidden" name="ord_date" value="'.date("d M Y").'">';
		// $form .='<input type="hidden" name="ord_shipname" value="'.$name.'">';
		// $form .='<input type="hidden" name="ord_mercref" value="Order '.$Itemid.'">';
		// $form .='<input type="hidden" name="ord_shipcountry" value="Malaysia">';
		// $form .='<input type="hidden" name="ord_totalamt" value="'.$amount.'" />';
		// $form .='<input type="hidden" name="ord_delcharges" value="0.00">';
		// $form .='<input type="hidden" name="ord_svccharges" value="0.00">';
		// $form .='<input type="hidden" name="ord_telephone" value="'.$phone.'">';
		// $form .='<input type="hidden" name="ord_email" value="'.$email.'">';
		// $form .='<input type="hidden" name="ord_returnURL" value="'.JRoute::_(JURI::root().'index.php?option=com_djclassifieds&task=paymentReturn&r=ok&id='.$item_id.$item_cid.'&Itemid='.$Itemid).'">';
		// $form .='<input type="hidden" name="ord_mercID" id="ord_mercID" value="'.$this->params["email_id"].'">';
		...
		$form .='<input type="hidden" name="ord_shipname" value="'.$name.'">';
		$form .='<input type="hidden" name="ord_shipcountry" value="Malaysia">';
		$form .='<input type="hidden" name="ord_mercref" value="Order '.$Itemid.rand(10,100).'">';
		$form .='<input type="hidden" name="ord_telephone" value="'.$phone.'">';
		$form .='<input type="hidden" name="ord_email" value="'.$email.'">';
		$form .='<input type="hidden" name="ord_delcharges" value="0.00">';
		$form .='<input type="hidden" name="ord_svccharges" value="0.00">';
		$form .='<input type="hidden" id="ord_mercID" name="ord_mercID" value="'.trim($this->params["email_id"]).'">';
		//$form .='<input type="hidden" id="ord_mercID" name="ord_mercID" value="80000321">';
		$form .='<input type="hidden" name="ord_returnURL" value="'.JRoute::_(JURI::root().'index.php?option=com_djclassifieds&task=paymentReturn&r=ok&id='.$item_id.$item_cid.'&Itemid='.$Itemid).'">';
		//$form .='<input type="submit" name="submit" value="Pay with Webcash">';


		//Backup Form from Paypal
	...
	?>
		<script type="text/javascript">
			...
			function callpayment(){
				var id = document.getElementById('ord_mercID').value ;
				if ( id > 0 && id != '' ) {
...
class webcash_ipn
{
	...
	var $timeout;
	var $error_email;
	function webcash_ipn($paypal_post_vars) {
		...
		if (!$fp) {
			...
			while (!feof($fp)) {
				...
				if ($this->send_time < time() - $this->timeout) {
					$this->error_out("Timed out waiting for a response from Webcash. ($this->timeout seconds)" , "");
				}
			}
			fclose( $fp );
		}
